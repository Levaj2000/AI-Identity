...
DISCLAIMER = "Engineering aid, not legal advice. Verify article citations against the OJ text with counsel."
...
def page_cover(c: canvas.Canvas) -> None:
    ...
    c.setFillColor(ICE_MUTED)
    c.setFont(FONT, 13)
    c.drawString(CONTENT_L + c.stringWidth("AI IDENTITY", FONT_B, 13) + 8, H - 130, "— Every agent leaves a trace.")
    ...
    y = draw_wrapped(
        ...
        CONTENT_L, y, FONT, 13, CONTENT_W - 60, 19, ICE_MUTED,
    )
    ...
    draw_wrapped(
        ...
        CONTENT_L + 20, y - 30, FONT, 10.5, CONTENT_W - 40, 15, ICE_MUTED,
    )
...
def page_deployer(c: canvas.Canvas) -> None:
    draw_background(c, 2)
    y = heading(c, "The deployer lane", "You run agents. That makes you a deployer — and the deployer duties get the least airtime.", H - MARGIN_T)
    ...
    y = draw_wrapped(
        ...
        CONTENT_L, y, FONT, 10.5, CONTENT_W, 15.5, ICE_MUTED,
    )
    y -= 18
    ...
    milestones = [
        ...
        ("2 Aug 2026", "General application: Annex III high-risk obligations and the Article 26 deployer duties. This is the one that lands on agent operators.", True),
    ...
    for date, text, hot in milestones:
        ...
        ny = draw_wrapped(c, text, rail_x + 100, y, FONT_B if hot else FONT, 10.5,
                          CONTENT_R - (rail_x + 100), 15, WHITE if hot else ICE_MUTED)
    ...
    y -= 20
    duties = [
        ("Art. 26(1)", "Operate each system per the provider's instructions — which means a record of how each agent was configured, and when that changed."),
        ("Art. 26(2)", "Assign human oversight: a named function with the competence and authority to intervene, and a record that intervention happened when it should have."),
        ("Art. 26(5)", "Monitor operation, and inform the provider or authorities when something looks like a serious incident."),
        ("Art. 26(6)", "Retain the automatically generated logs for at least six months — longer where other EU or national law says so."),
    ]
    for art, text in duties:
        c.setFillColor(AMBER)
        c.setFont(FONT_B, 10.5)
        c.drawString(CONTENT_L, y, art)
        y = draw_wrapped(c, text, CONTENT_L + 78, y, FONT, 10.5, CONTENT_R - (CONTENT_L + 78), 15, ICE_MUTED)
        y -= 10
    ...
    draw_wrapped(
        ...
        CONTENT_L, y - 4, FONT_O, 10.5, CONTENT_W, 15.5, ICE,
    )
    c.showPage()
...
def page_evidence_test(c: canvas.Canvas) -> None:
    ...
    c.setFillColor(WHITE)
    c.setFont(FONT_B, 14)
    c.drawString(CONTENT_L + 20, y - 12, '"How do you know this record wasn\'t edited after the fact?"')
    y -= 62
    ...
    questions = [
        ...
        ("2", "Can a third party verify the sequence of events is intact — offline, without trusting you or your vendor?"),
        ("3", "Which of your agents fall under an Annex III high-risk category? Is that a query today, or a quarterly archaeology project?"),
        ("4", "Who approved the last high-impact agent action — and where is the approval, denial, or expiry recorded?"),
        ("5", "If your logging vendor disappeared tomorrow, would the evidence you owe under Article 26(6) survive?"),
    ]
    for num, text in questions:
        c.setFillColor(AMBER)
        c.setFont(FONT_B, 13)
        c.drawString(CONTENT_L, y, num)
        ny = draw_wrapped(c, text, CONTENT_L + 24, y, FONT, 11, CONTENT_R - (CONTENT_L + 24), 15.5, WHITE)
        c.setStrokeColor(NAVY_RULE)
        c.setLineWidth(0.75)
    ...
    draw_wrapped(
        ...
        CONTENT_L, y, FONT_O, 10.5, CONTENT_W, 15.5, ICE,
    )
    c.showPage()
...
def worksheet_page(c: canvas.Canvas, page_num: int, rows, first: bool) -> None:
    ...
    if first:
        ...
        y = heading(c, "The worksheet", "Ten records the Act expects you to produce (cont.)", H - MARGIN_T)
...
def page_evidence_set(c: canvas.Canvas) -> None:
    draw_background(c, 6)
    y = heading(c, "What good looks like", "One bundle, one article per artifact, verifiable without us", H - MARGIN_T)
    ...
    y -= 20
    limits = [
        ("Article 10 data governance is the provider's problem.", "Upstream training-data governance sits outside a deployer platform's visibility. Our Annex IV export says so explicitly rather than fabricating evidence to fill the section."),
        ("There is no EU AI Act certificate to wave.", "Conformity assessment bodies are still standing up. What can exist today is architecture that produces the required records in the required shape — and an export a regulator can verify without trusting the vendor."),
    ]
    ...
    draw_wrapped(
        ...
        CONTENT_L + 20, y - 34, FONT, 10, CONTENT_W - 40, 14.5, ICE_MUTED,
    )
    c.setFillColor(ICE)
    c.setFont(FONT_B, 12)
...
def main() -> None:
    ...
    c.setSubject("The records Articles 12 and 26 require of AI agent deployers, and a worksheet to locate each one.")
    page_cover(c)
    page_deployer(c)
