"""Build a Kevin Pentecost / Standard Motor Co briefing deck.

Generates a clean, presentation-ready pptx that the founder can lean on
during a face-to-face conversation with the Security Director. Style
matches the existing aiidentitytraining.pptx (navy on white, colored
left-accent bars on cards, sans-serif).

Run: python3 build_standard_motor_deck.py
Output: ../briefing-standard-motor-2026-04-28.pptx
"""
...
DECK_FOOTER = "AI Identity  •  Briefing for Standard Motor Co"
TODAY = "April 28, 2026"

OUT = Path(__file__).resolve().parent.parent / "briefing-standard-motor-2026-04-28.pptx"
...
add_text(
    ...
    [
        (
            f"Briefing for Kevin Pentecost  ·  Standard Motor Co  ·  {TODAY}",
...
add_text(
    ...
    [
        (
            "Internal — founder reference for the conversation",
            dict(size=10, color=RGBColor(0x6F, 0x7D, 0x9C), align="left"),
        )
    ],
)
...
s = prs.slides.add_slide(blank)
add_slide_title(s, "The 30-second pitch", "If I only get one sentence out, this is it")
...
add_text(
    ...
    Inches(11.7),
    Inches(0.4),
    [("What I'd love to walk you through today:", dict(size=14, color=GREY, bold=True))],
)
for i, line in enumerate(
    [
        "1.  The problem every security team is about to inherit (5 min)",
        "2.  How teams cope today, and why it doesn't actually work (5 min)",
        "3.  How AI Identity solves it — gateway, dual-key, forensic chain (10 min)",
        "4.  Two scenarios from your world: parts ordering, customer service refunds (10 min)",
        "5.  What a Standard Motor pilot could look like (5 min)",
    ]
):
    add_text(
        s,
...
add_text(
    ...
    [
        (
            ...
            "external party (us), bound to a hardware-rooted key the agent never holds.",
...
add_text(
    ...
    [
        (
            "Honest framing matters with auditors. Overclaiming is how trust gets burned.",
...
add_slide_title(
    ...
    "How Kevin's team would actually use AI Identity day-to-day.",
)
steps = [
    (
        "Day 1",
        "Engineer registers a new agent.",
        "Single API call → returns aid_sk_ once. Permissions default to deny. Engineer writes a policy: 'this agent can call /reorder, max $5K/day, supplier in {approved-list}.' Reviewed and approved by Kevin's team in the dashboard.",
    ),
    (
        "Day 1, 30 mins later",
    ...
    (
        "Day 14",
        "An anomaly fires.",
        "Agent suddenly tries 50 reorders in 60 seconds. Anomaly detection flags it; auto-revoke kicks in; Kevin gets paged with the agent ID and the last 100 audit rows pre-filtered.",
    ),
    (
        "Quarterly",
...
# 14. Likely objections ---------------------------------------------------
s = prs.slides.add_slide(blank)
add_slide_title(
    s,
    "Likely objections — and the honest answers",
    "What I'd expect Kevin to push on, and how I'd respond.",
)
qa = [
    (
        "“Why not just rotate keys per agent in our existing IAM?”",
        "You can. But Okta/Entra are session-oriented; an agent isn't a session, it's a long-lived process making thousands of decisions. You'd end up building the policy engine and the forensic chain on top of them — that's a quarter of platform work for something we sell as a product.",
    ),
    (
        "“What's the latency on the hot path?”",
        "Gateway target is <50ms p99 per enforcement decision. Verification of attestations is fully offline — your runtime never blocks on us. Scale your agents to a million calls/sec; we don't care.",
    ),
    (
        "“How does this work with our SIEM?”",
        "Audit export API. Push to Splunk, Sentinel, or a custom sink. Our log is the source of truth; your SIEM is the analytics layer.",
    ),
    (
        "“What if AI Identity itself gets compromised?”",
        "The HMAC chain key and the KMS signing key are held in different systems — DB and HSM. Forging history requires both. The KMS key has never been seen by an AI Identity process. Auditors can verify against the HSM directly via gcloud.",
    ),
    (
        "“What does this cost?”",
        "Free $0, Pro $79, Business $299, Business+ $599 — published rate-card. Enterprise is $1,500/mo multi-tenant or $3,000/mo dedicated VPC. Standard Motor specifically: 90 days of Enterprise free as our founding customer; Year 1+ locked at the rate-card Enterprise price for 24 months.",
    ),
]
y = Inches(2.05)
for q, a in qa:
...
add_text(
    ...
    Inches(11.3),
    Inches(0.4),
    [("THE ASK", dict(size=14, bold=True, color=ACCENT_AMBER))],
)
add_text(
    s,
    Inches(1.0),
    ...
    [
        (
            "Standard Motor as our founding Enterprise customer.",
...
add_text(
    ...
    [
        (
            "90 days of the Enterprise tier — free. No commitment. Walk away at day 90 if it isn't a fit.",
            dict(size=14, color=RGBColor(0xC9, 0xD4, 0xE6)),
        )
    ],
)
...
add_text(
    ...
    Inches(11.3),
    Inches(0.35),
    [("ENTERPRISE TIER — INCLUDED FOR 90 DAYS", dict(size=11, bold=True, color=ACCENT_AMBER))],
)
ent_inline = (
    "·  Unlimited agents     ·  Unlimited requests     ·  Unlimited audit retention     "
    "·  Full SSO + SAML / SCIM     ·  Compliance evidence export     ·  Team roles + agent assignments     "
    "·  Human-in-the-loop review     ·  On-premise / VPC deployment option     ·  Dedicated support + SLA"
)
add_text(
    s,
    Inches(1.0),
    ...
    Inches(11.3),
    Inches(0.8),
    [(ent_inline, dict(size=11, color=RGBColor(0xE9, 0xEE, 0xF8)))],
)

# Founding customer perks
add_text(
    s,
    Inches(1.0),
    Inches(4.55),
    Inches(11.3),
    Inches(0.35),
    [("WHAT FOUNDING-CUSTOMER STATUS ADDS", dict(size=11, bold=True, color=ACCENT_AMBER))],
)
perks = [
    "Direct founder line — same-day responsiveness.",
    "First three policies authored with you, not by you.",
    "Real influence on the roadmap during evaluation.",
    "Pricing locked at evaluation rates if you continue past day 90.",
    "Optional named launch case study, with your sign-off on every quote.",
]
for i, line in enumerate(perks):
    add_text(
        s,
        Inches(1.0),
        Inches(4.92) + Inches(0.32) * i,
        Inches(11.3),
        Inches(0.32),
        [(f"·  {line}", dict(size=12, color=RGBColor(0xE9, 0xEE, 0xF8)))],
    )
...
add_text(
    ...
    Inches(11.3),
    Inches(0.35),
    [("AT DAY 90 — TRANSPARENT PRICING", dict(size=11, bold=True, color=ACCENT_AMBER))],
)
add_text(
    s,
    Inches(1.0),
    ...
    [
        (
            "Multi-tenant Enterprise: $1,500/mo, locked 24 months  ·  Dedicated VPC: $3,000/mo, locked 24 months  ·  Or step down: Pro $79 / Business $299 / Business+ $599.",
            dict(size=11, color=RGBColor(0xE9, 0xEE, 0xF8)),
        )
    ],
)
add_text(
    ...
    [
        (
            f"{DECK_FOOTER}  •  15 / {SLIDES_PLANNED}",
            dict(size=9, color=RGBColor(0x6F, 0x7D, 0x9C), align="right"),
        )
    ],
)
