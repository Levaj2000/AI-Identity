...
def test_backlog_drain_never_reruns_high_water_mark_lookup(
    ...
    """A multi-batch drain must not run the max(last_audit_id) lookup at all.

    The candidate query resolves each org's high-water mark up front and threads
    it into the drain as ``after_id``, so ``_last_anchored_id`` is never called —
    not per batch (the N+1 Sentry flagged) nor even once per org.
    """
    ...
    # Spy on the high-water-mark lookup create_checkpoint falls back to.
    lookups: list = []
    ...
    assert body["events_anchored"] == 12
    # The candidate query supplies the mark, so the per-org fallback never fires.
    assert lookups == []
