...
def emit_checkpoints(
    ...
    # trigger create_checkpoint's max()-lookup + un-anchored fetch. The query
    # also hands back each org's high-water mark, which we thread straight into
    # create_checkpoint as ``last_id`` so the drain skips that DB lookup too.
    hwm = (
        select(
    ...
    for org_id, last_anchored_id in candidates:
        ...
        # is never re-run — not once per batch (the N+1 Sentry flagged) and not
        # even once per org — the candidate query already resolved it.
        last_id = last_anchored_id
