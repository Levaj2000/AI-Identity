...
### Fixed
- _Forensics:_ the Evidence Anchor checkpoint cron no longer re-runs the `max(last_audit_id)` high-water-mark lookup once per batch while draining an org's backlog — it resolves the mark once per org and threads it forward. Removes the N+1 Sentry flagged on `/api/internal/evidence-anchor/checkpoint` and trims the endpoint's P95 latency. Internal tooling only — no product impact. (#408)
- _Forensics:_ the same checkpoint cron no longer runs `SELECT DISTINCT org_id` over the whole `audit_log` table every tick, then probes every org. It now selects only orgs whose latest `audit_log.id` is ahead of their checkpoint high-water mark — one set-based query that skips idle orgs entirely and reuses the same mark to seed each drain, so the `max(last_audit_id)` lookup never runs separately. The remaining P95-latency lever on the anchor endpoint. The run summary field `orgs_scanned` is now `orgs_with_backlog`. Internal tooling only — no product impact. (#408)
