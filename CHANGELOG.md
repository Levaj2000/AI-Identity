...
### Added
- **Evidence Anchor primitive (spike, internal)** — `common/forensic/merkle.py` (RFC 6962 Merkle tree + O(log N) offline inclusion verifier) and `common/forensic/anchor_checkpoint.py` (DSSE-signed Merkle checkpoint over a batch of audit `entry_hash` values). Lets a third party prove one audit entry is committed using only a published ECDSA-P256 public key + SHA-256 — no shared secret, unlike the HMAC chain. Reuses the existing KMS-backed forensic signer + JWKS; not yet wired into exports. (#406)
