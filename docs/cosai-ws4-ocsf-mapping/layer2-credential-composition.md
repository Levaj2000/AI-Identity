...
**Status:** discussion note, not a proposal. Companion to the field-level fit
table between Verifiable Intent's L2 credential and the CMF delegation
extension.
...
| Biscuit | `biscuit-auth`, as minted and verified by the AI Identity mandate service | platform implementation, 0.4.0 onward |
| Mandate document | The AI Identity mandate document and its constraint registry, schema 1.2 | platform implementation |
...
| Bounds vocabulary | Registered, typed, quantitative | Whatever Datalog facts both ends agreed on | Namespaced types in the constraint registry |
...
note our own minting code carries. A Biscuit's `subject` check binds the token to
