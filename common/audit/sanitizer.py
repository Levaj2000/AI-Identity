...
PII_FIELD_BLOCKLIST: frozenset[str] = frozenset({
    ...
    "ssn", "social_security", "national_id", "passport", "passport_number",
    "date_of_birth", "dob", "birthday",
    # Network / tracking
    "ip", "ip_address", "client_ip", "remote_addr", "x_forwarded_for",
    "user_agent", "ua", "referer", "referrer",
    # Auth / secrets
    "authorization", "auth_token", "bearer", "token",
    "api_key", "secret", "password", "credential", "credentials",
    "cookie", "cookies", "session_id", "session_token",
...
_PII_PATTERNS = [
    ...
    re.compile(r".*(?:auth_token|access_token|refresh_token|bearer_token|id_token|session_token|api_token).*", re.IGNORECASE),
    # Match body/header only as field containers, not metrics
    re.compile(r".*(?:request_body|response_body|request_header|response_header|raw_body|raw_header).*", re.IGNORECASE),
]
