...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={handleCopy}
      className="absolute right-3 top-3 rounded-md border border-white/10 bg-[#1a1a1d] px-2.5 py-1 text-xs font-medium text-gray-400 transition-colors hover:bg-[#2a2a2d] hover:text-white"
    >
      {copied ? "Copied!" : "Copy"}
    </button>
  );
    ...
    <div className="my-4 rounded-xl border border-white/5 overflow-hidden">
      {title && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-[#111113] border-b border-white/5">
          <span className="text-xs font-mono text-[#F59E0B]/80">
            {language}
          </span>
      ...
      <div className="relative">
        <pre className="overflow-x-auto bg-[#111113] p-4 text-sm leading-relaxed">
      ...
      return (
        <div key={index} className="space-y-8 my-6">
          {block.items.map((step, i) => (
            <div
              key={i}
              className="border-l-2 border-[#F59E0B]/30 pl-6"
            >
      ...
      return (
        ...
        >
          {block.items.map((card, i) => (
            <div
              key={i}
              className="rounded-xl border border-white/5 bg-[#111113]/60 p-6"
            >
        ...
        <div key={index} className="grid grid-cols-1 sm:grid-cols-2 gap-4 my-6">
          {block.items.map((link, i) => (
            <a
              ...
              className="group rounded-xl border border-white/5 bg-[#111113]/60 p-6 hover:border-[#F59E0B]/30 transition-colors"
            >
              <div className="flex items-center gap-2 mb-2">
                <h3 className="text-base font-semibold text-white group-hover:text-[#F59E0B] transition-colors">
                  {link.label}
                </h3>
                <svg
                  width="14"
                  ...
                  stroke="currentColor"
                  strokeWidth="2"
                  className="text-gray-500 group-hover:text-[#F59E0B] transition-colors"
                >
      ...
      <div className="pt-[72px]">
        ...
        <section className="px-6 md:px-12 pt-16 pb-10">
          <div className="max-w-[1100px] mx-auto">
            <div className="mb-2 px-3 py-1 inline-block border border-[#F59E0B]/30 rounded-full text-[#F59E0B] text-xs font-medium tracking-widest uppercase">
              Documentation
            </div>
            <h1 className="text-4xl md:text-5xl font-bold text-white mt-4 mb-4">
              Build with <span className="text-[#F59E0B]">AI Identity</span>
            </h1>
        ...
        <div className="max-w-[1100px] mx-auto px-6 md:px-12 pb-24 flex gap-12">
            ...
            <nav className="sticky top-[120px] space-y-1">
                ...
                <button
                  ...
                  className={`block w-full text-left px-3 py-1.5 rounded-lg text-sm transition-colors ${
                    activeSection === section.id
                      ? "text-[#F59E0B] bg-[#F59E0B]/5 font-medium"
              ...
              <div className="border-t border-white/5 mt-4 pt-4">
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block px-3 py-1.5 text-sm text-gray-500 hover:text-[#F59E0B] transition-colors"
                >
                  API Reference &rarr;
                </a>
                <a
          ...
          <main className="flex-1 min-w-0">
            {/* Mobile TOC */}
            <div className="md:hidden mb-8 rounded-xl border border-white/5 bg-[#111113]/60 p-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">
                On this page
              </p>
