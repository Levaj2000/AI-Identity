...
const securityFeatures = [
  {
    ...
    description: "Every request is authenticated and authorized before processing. No implicit trust, no shortcuts. The gateway validates agent identity, checks policy, and logs the result before any request reaches an LLM provider.",
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>,
  },
  {
    title: "Fail-Closed by Default",
    description: "If something goes wrong during policy evaluation — timeout, error, ambiguity — the request is denied. Agents cannot bypass controls, even during partial outages. Security is the default, not the exception.",
  ...
  },
  {
    ...
    description: "LLM provider API keys are stored encrypted at rest (AES-256) and only decrypted in-memory at request time. Agents never see or handle raw provider credentials.",
  ...
  },
  {
    ...
    description: "Every audit record is cryptographically chained using HMAC-SHA256. Each entry includes the hash of the previous entry, creating an unbroken chain. Alter one record and the chain breaks — making tampering detectable and provable.",
  ...
  },
  {
    ...
    description: "Row-level security at the database layer ensures complete data isolation between tenants. One customer's agents, policies, keys, and audit logs can never leak into another's — even in the event of an application-level vulnerability.",
    icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>,
  },
  {
    title: "Key Separation",
    description: "Organization keys (for management) and agent keys (for LLM requests) are fully separated. Compromising an agent's runtime key does not grant access to management operations or other agents.",
...
const compliance = [
  { framework: "SOC 2 Type II", status: "Architecture aligned", desc: "Logical access controls, tamper-evident audit trail, encryption, tenant isolation." },
  { framework: "NIST AI RMF", status: "Framework supported", desc: "Agent observability, policy governance, cryptographic integrity, fail-closed design." },
  { framework: "EU AI Act", status: "High-risk ready", desc: "Human oversight, traceability, transparent enforcement, audit log export." },
];
      ...
      <section className="pt-32 pb-16 px-6 md:px-12">
        <div className="max-w-[900px] mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-white mb-6 leading-tight">
            Enterprise-Grade{" "}
            <span className="text-[rgb(166,218,255)]">Security</span>{" "}
            for AI Agents
          </h1>
          <p className="text-lg text-gray-400 max-w-[640px] mx-auto leading-relaxed">
            AI Identity is built with security as the foundation, not an afterthought.
            Zero-trust architecture, encrypted credential management, and tamper-evident
            audit trails — designed for regulated industries.
          </p>
        </div>
      </section>
        ...
        <div className="max-w-[800px] mx-auto">
          <div className="bg-[rgb(166,218,255)]/5 border border-[rgb(166,218,255)]/20 rounded-2xl p-8 text-center">
            ...
            <p className="text-sm text-gray-400 mb-6 max-w-[500px] mx-auto">
              Read our architecture documentation or schedule a call with our team
              to discuss how AI Identity fits your security requirements.
            </p>
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <Link to="/architecture" className="inline-flex items-center gap-2 px-6 py-3 bg-[rgb(166,218,255)] text-[rgb(4,7,13)] font-semibold rounded-xl hover:bg-[rgb(166,218,255)]/80 transition-colors">
                View Architecture
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
              </Link>
              <Link to="/contact" className="inline-flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 text-white font-medium rounded-xl hover:bg-white/10 transition-colors">
                Schedule a Call
              </Link>
            </div>
          </div>
