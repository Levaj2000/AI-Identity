...
const openQuestions = [
  {
    ...
    body: "Anthropic and Bedrock-Claude do not expose token logprobs. Forensic attestations rely on response provenance (model_id, system fingerprint) — but this is weaker than logprob-backed replay. Provider API changes are now tracked against the AI Identity platform implementation.",
  },
  {
    n: "9.2",
    title: "Multimodal context capture",
    body: "Image, audio, and video inputs are out of scope for v1.0. The draft sketched a v1.1 approach — capture content-hash + MIME + size rather than raw bytes, with optional vendor-specific perceptual hashes — now carried forward on the platform roadmap.",
  },
  {
    n: "9.3",
    title: "Cross-organization federation",
    body: "Agent identity assertions across trust domains depend on IETF AIP draft maturity. v1.0 assumes a single-org trust domain; a federation profile was deferred pending the IETF work.",
  },
  {
    n: "9.4",
...
export default function SpecContent() {
  return (
    <>
      ...
      <section className="pt-32 pb-12 px-6 md:px-12">
        <div className="max-w-[900px] mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[rgb(166,218,255)]/10 border border-[rgb(166,218,255)]/20 rounded-full mb-8">
            ...
            <span className="text-[rgba(213,219,230,0.5)] text-xs">Draft v1.0 published 2026-05-15 · graduated upstream 2026-08</span>
          </div>
          ...
          <p className="text-lg text-gray-400 max-w-[760px] mx-auto leading-relaxed">
            An open reference draft for tamper-evident, cryptographically-signed audit trails of autonomous AI agents — and the working draft behind what is now shipped in OCSF 1.9.0. The spec&apos;s core primitives, the <code className="text-[rgb(166,218,255)] text-base">attestation</code> object and the <code className="text-[rgb(166,218,255)] text-base">record_integrity</code> profile, were contributed upstream and are now part of the OCSF schema. The normative home for this work is OCSF; the AI Identity platform is the production reference implementation; the archived draft remains the historical record.
          </p>
          <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
            ...
            <a
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 text-white font-medium rounded-xl hover:bg-white/10 transition-colors"
            >
              Archived draft (historical record)
            </a>
          </div>
        </div>
      </section>
      ...
      {/* Graduation timeline */}
      <section className="py-12 px-6 md:px-12 border-t border-[rgba(216,231,242,0.05)]">
        <div className="max-w-[1000px] mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {[
              {
                date: "May 2026",
                title: "Draft v1.0 published",
                detail: "Open reference draft released for public review under CC-BY-4.0.",
              },
              {
                date: "Jul 2026",
                title: "Contributed upstream",
                detail: (
                  <>
                    <a href="https://github.com/ocsf/ocsf-schema/pull/1661" target="_blank" rel="noopener noreferrer" className="text-[rgb(166,218,255)] hover:underline">ocsf-schema#1661</a>
                    {" "}merged: the attestation object + record_integrity profile enter the OCSF schema.
                  </>
                ),
              },
              {
                date: "Aug 2026",
                title: "Shipped in OCSF 1.9.0",
                detail: (
                  <>
                    Released in the{" "}
              ...
              },
              {
                ...
                detail: "The spec repo is archived as the historical record; new schema work happens upstream.",
              },
            ].map((step) => (
              <div key={step.title} className="p-5 rounded-2xl border border-[rgba(216,231,242,0.07)] bg-white/[0.02]">
                <p className="text-[10px] uppercase tracking-[0.2em] text-[rgb(166,218,255)] mb-2">{step.date}</p>
                <h3 className="text-sm font-semibold text-white mb-2">{step.title}</h3>
                <p className="text-xs text-[rgba(213,219,230,0.7)] leading-relaxed">{step.detail}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[800px] mx-auto text-center">
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            <a
              ...
              className="inline-flex items-center gap-2 px-6 py-3 bg-[rgb(166,218,255)] text-[rgb(4,7,13)] font-semibold rounded-xl hover:bg-[rgb(166,218,255)]/80 transition-colors"
            >
              Contribute upstream in ocsf/ocsf-schema
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
            </a>
            <Link
          ...
          <p className="mt-8 text-xs text-[rgba(213,219,230,0.45)]">
            ...
            <a
              ...
              className="text-[rgb(166,218,255)] hover:underline"
            >
              forensic-audit-trail-spec
            </a>
            {" · "}
            Draft license: CC-BY-4.0 (content) + Apache-2.0 (reference impl)
          </p>
        </div>
      </section>
    </>
