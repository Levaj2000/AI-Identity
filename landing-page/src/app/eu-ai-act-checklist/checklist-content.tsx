...
function AnswerButton({
  ...
  const colors: Record<Answer, { border: string; bg: string; text: string; selectedText: string }> = {
    ...
    "not-sure": { border: "#A6DAFF", bg: "#A6DAFF", text: "#A6DAFF", selectedText: "#0a2540" },
  };
...
export default function EUAIActChecklistContent() {
      ...
      <section className="pt-32 pb-16 px-6 md:px-12">
        <div className="max-w-[900px] mx-auto">
          ...
          <motion.div
            ...
            className="bg-[rgb(16,19,28)]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 mb-12"
          >
            ...
            <div className="w-full h-2 bg-white/5 rounded-full overflow-hidden mb-3">
              <div
                ...
                style={{ width: `${progressPercent}%`, backgroundColor: currentRange.color }}
              />
            </div>
            <div className="flex items-center justify-between flex-wrap gap-2">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${currentRange.color}20`, color: currentRange.color }}>
                  {currentRange.label}
                </span>
              ...
              <div className="flex items-center gap-3 text-xs font-mono">
                ...
                <span className="text-gray-600">&middot;</span>
                <span className="text-[#A6DAFF]">{notSureCount} Not Sure</span>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
            {sections.map((section, sectionIdx) => (
              <motion.div
                ...
                className="bg-[rgb(16,19,28)]/80 backdrop-blur-xl border border-white/5 rounded-2xl overflow-hidden"
              >
                <div className="px-6 py-4 border-b border-white/5 flex items-center justify-between">
                  ...
                  <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: `${section.pillarColor}15`, color: section.pillarColor }}>
                ...
                <div className="px-6 py-3">
                  {section.items.map((checkItem) => (
                    <div key={checkItem.id} className="checklist-item py-4 border-b border-white/[0.03] last:border-0">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-gray-200 leading-relaxed">{checkItem.question}</p>
                          <p className="text-xs text-gray-500 mt-1 leading-relaxed">{checkItem.helper}</p>
                        </div>
                        <div className="flex items-center gap-2 flex-shrink-0 pt-0.5">
                          <AnswerButton label="Yes" value="yes" selected={answers[checkItem.id] === "yes"} onClick={() => setAnswer(checkItem.id, "yes")} />
                          <AnswerButton label="No" value="no" selected={answers[checkItem.id] === "no"} onClick={() => setAnswer(checkItem.id, "no")} />
                          <AnswerButton label="Not Sure" value="not-sure" selected={answers[checkItem.id] === "not-sure"} onClick={() => setAnswer(checkItem.id, "not-sure")} />
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          {/* Results Summary */}
          {allAnswered && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mt-12 bg-[rgb(16,19,28)]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6">
              ...
              <p className="text-sm text-gray-400 mb-6">
                ...
                <span className="text-[#A6DAFF] font-semibold">{notSureCount} Not Sure</span> out of {totalItems} questions.
              </p>
              ...
              {sectionGaps.length > 0 ? (
                <>
                  ...
                  <div className="space-y-2 mb-6">
                    {sectionGaps.map((gap) => (
                      <div key={gap.title} className="flex items-center justify-between p-3 rounded-xl border border-white/5">
                        <span className="text-sm text-gray-300">{gap.title}</span>
                        <div className="flex items-center gap-3 text-xs font-mono">
                          {gap.noCount > 0 && <span className="text-[#EF4444]">{gap.noCount} No</span>}
                          {gap.notSureCount > 0 && <span className="text-[#A6DAFF]">{gap.notSureCount} Not Sure</span>}
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="border-t border-white/5 pt-6">
                <a href="/request-services" className="inline-flex items-center gap-2 px-6 py-3 bg-[rgb(166,218,255)] text-[rgb(4,7,13)] font-semibold rounded-xl hover:bg-[rgb(166,218,255)]/80 transition-colors">
                  AI Identity can help close these gaps. Request services
          ...
          {/* Scoring Guide */}
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mt-12 bg-[rgb(16,19,28)]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6 print-break">
            <h2 className="text-lg font-semibold text-white mb-4">Scoring Guide</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {scoreRanges.map((range) => (
                <div key={range.label} className="flex items-start gap-3 p-4 rounded-xl border border-white/5">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0" style={{ backgroundColor: `${range.color}15`, color: range.color }}>
                    {range.min}-{range.max}
                  </div>
                  <div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mt-12 print-break">
            <div className="text-center mb-8">
              <h2 className="text-2xl md:text-3xl font-bold text-white mb-3">How AI Identity Helps</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {pillarMapping.map((p) => (
                <div key={p.pillar} className="bg-[rgb(16,19,28)]/80 backdrop-blur-xl border border-white/5 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: `${p.color}15` }}>
                      {p.icon === "key" && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={p.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4" /></svg>}
                      {p.icon === "shield" && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={p.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>}
                      {p.icon === "check" && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={p.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14 2 14 8 20 8" /><polyline points="9 15 11 17 15 13" /></svg>}
                      {p.icon === "search" && <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={p.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8" /><line x1="21" y1="21" x2="16.65" y2="16.65" /></svg>}
                    </div>
                    <div>
                      <h3 className="text-sm font-semibold text-white">{p.pillar}</h3>
                      <span className="text-xs text-gray-500">Covers: {p.sections.join(", ")}</span>
                    </div>
                  </div>
                  <div className="space-y-2">
                    {p.automated.map((a) => (
                      <div key={a} className="flex items-center gap-2">
                        <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: p.color }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="20 6 9 17 4 12" /></svg>
          ...
          {/* Disclaimer */}
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mt-8 p-4 border border-white/5 rounded-xl">
          ...
          {/* CTA */}
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }} className="mt-12 mb-8 text-center no-print">
            <div className="bg-gradient-to-br from-[rgb(166,218,255)]/10 to-[rgb(16,19,28)]/80 backdrop-blur-xl border border-[rgb(166,218,255)]/20 rounded-2xl p-8">
              ...
              <div className="flex items-center justify-center gap-4 flex-wrap">
                ...
                <a href="/request-services" className="inline-flex items-center gap-2 px-6 py-3 bg-white/5 border border-white/10 text-white font-medium rounded-xl hover:bg-white/10 transition-colors">
